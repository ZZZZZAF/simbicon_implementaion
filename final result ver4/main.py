    ...
    def handle(self, ea, aa):
        if ea.getEventType() == dart.gui.osg.GUIEventAdapter.KEYDOWN:
            ...
            if ea.getKey() == dart.gui.osg.GUIEventAdapter.KEY_Z:
                self.node.enableCameraBehind()

            if ea.getKey() == dart.gui.osg.GUIEventAdapter.KEY_X:
                self.node.enableCameraLeft()
...
class MyWorldNode(dart.gui.osg.RealTimeWorldNode):
    def __init__(self, world, skel):
        ...
        temp[self.torso_ry] = 0.2
        temp[self.left_thigh_ry] = -0.5
        temp[self.left_shin] = 1.1
        temp[self.left_heel] = -0.6
        temp[self.left_bicep] = 0.3
        temp[self.left_forearm] = -0.2
        temp[self.right_shin] = 0.05
        temp[self.right_heel] = 0.0
        temp[self.right_bicep] = -0.3
        temp[self.right_forearm] = -0.2
        self.state1 = temp
        ...
        self.Kp_torso_sagital = 900
        self.Kd_torso_sagital = 90

        self.Kp_torso_coronal = 900
        self.Kd_torso_coronal = 90

        self.Kp_thigh_sagital = 600
        self.Kd_thigh_sagital = 60

        self.Kp_thigh_coronal = 600
        self.Kd_thigh_coronal = 60
    ...
    def setCameraPositionBehind(self):
    ...
    def setCameraPositionLeft(self):
        comFrame = self.getCOMFrame()
        eyePosition = comFrame.multiply([0,5,1])
        targetPoint = comFrame.translation()
        self.viewer.setCameraHomePosition(eyePosition,
                                          targetPoint,
                                          [0,0,1])
    ...
    def enableCameraBehind(self):
        if self.cameraLeft == True and self.freeCamera == False and self.cameraBehind == False:
            self.cameraLeft = False
            self.freeCamera = False
            self.cameraBehind = True
        elif self.cameraLeft == False and self.freeCamera == False and self.cameraBehind == True:
            self.cameraLeft = False
            self.freeCamera = True
            self.cameraBehind = False
        elif self.cameraLeft == False and self.freeCamera == True and self.cameraBehind == False:
            self.cameraLeft = False
            self.freeCamera = False
            self.cameraBehind = True

    def enableCameraLeft(self):
        if self.cameraLeft == True and self.freeCamera == False and self.cameraBehind == False:
            self.cameraLeft = False
            self.freeCamera = True
            self.cameraBehind = False
    ...
    def customPreStep(self):
        ...
        if self.current_controller == 'stand':
            ...
            p = np.matmul(-self.Kp_stand, q - self.state0)
            ...
            if self.freeCamera == False:
                if self.cameraBehind == True:
                    self.setCameraPositionBehind()
                elif self.cameraLeft == True:
                    self.setCameraPositionLeft()
...
def main():
    pathname = "/./home/sizheng/桌面/final result ver4"
    world = dart.simulation.World()

    urdfParser = dart.utils.DartLoader()
    robot = urdfParser.parseSkeleton(pathname + "/fullbody.urdf")
    ...
    ground1 = urdfParser.parseSkeleton(pathname + "/ground1.urdf")
    ground2 = urdfParser.parseSkeleton(pathname + "/ground2.urdf")
    ground3 = urdfParser.parseSkeleton(pathname + "/ground3.urdf")
    ground4 = urdfParser.parseSkeleton(pathname + "/ground4.urdf")

    box1 = urdfParser.parseSkeleton(pathname + "/box1.urdf")
    box2 = urdfParser.parseSkeleton(pathname + "/box2.urdf")
    box3 = urdfParser.parseSkeleton(pathname + "/box3.urdf")
    box4 = urdfParser.parseSkeleton(pathname + "/box4.urdf")
    box5 = urdfParser.parseSkeleton(pathname + "/box5.urdf")
    box6 = urdfParser.parseSkeleton(pathname + "/box6.urdf")
    box7 = urdfParser.parseSkeleton(pathname + "/box7.urdf")

    starting_point_ball = urdfParser.parseSkeleton(pathname + "/ball1.urdf")
    ending_point_ball = urdfParser.parseSkeleton(pathname + "/ball2.urdf")

    windmill1 = urdfParser.parseSkeleton(pathname + "/windmill1.urdf")
    windmill2 = urdfParser.parseSkeleton(pathname + "/windmill2.urdf")
    windmill3 = urdfParser.parseSkeleton(pathname + "/windmill3.urdf")

    slope1 = urdfParser.parseSkeleton(pathname + "/slope1.urdf")
    slope2 = urdfParser.parseSkeleton(pathname + "/slope2.urdf")
    slope3 = urdfParser.parseSkeleton(pathname + "/slope3.urdf")
    slope4 = urdfParser.parseSkeleton(pathname + "/slope4.urdf")

    base = urdfParser.parseSkeleton(pathname + "/base1.urdf")
    board = urdfParser.parseSkeleton(pathname + "/board1.urdf")

    large_pendulum = urdfParser.parseSkeleton(pathname + "/large pendulum.urdf")
